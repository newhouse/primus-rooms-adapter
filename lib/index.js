...
Adapter.prototype.del = function del(id, room, fn) {
  ...
  if (room) {
    if (ids[id] && ids[id][room]) prune(room);
  } else {
...
Adapter.prototype.broadcast = function broadcast(data, opts, clients) {
  ...
  // Go through each room provided.
  rooms.forEach(function each(room) {
    // Get its IDs
    var ids = adapter.rooms[room];
    // Immediately send to them if there were any.
    if (ids) send(ids);

    // Pass this room to the wildcard plugin to see if it contains any
    // wildcard characters that can match any of the adapter's rooms
    adapter.wildcard.match(room, adapter.rooms, function match(matchedRoom) {
      // Get this matched room's IDs
      ids = adapter.rooms[matchedRoom];
      // Send to them if there were any.
      if (ids) send(ids);
    });
  });
